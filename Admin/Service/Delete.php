<?php
    ...
    function handleDelete($connectionn) {
        ...
        if ($id > 0) {
            $stmt = $connectionn->prepare("DELETE FROM services WHERE id = ?");
            if ($stmt === false) {
    ...
    function handleDeactivate($connection) {
        ...
        if ($id > 0) {
            $stmt = $connection->prepare("UPDATE services SET status = 0 WHERE id = ?");
            $stmt->bind_param("i", $id); // "i" means the parameter is an integer
    ...
    function handleActivate($connection) {
        ...
        if ($id > 0) {
            $stmt = $connection->prepare("UPDATE services SET status = 1 WHERE id = ?");
            $stmt->bind_param("i", $id); // "i" means the parameter is an integer
