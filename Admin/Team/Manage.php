...
if (isset($_GET['action']) && $_GET['action'] === 'get_data') {
    ...
    $sql = "SELECT name, position, role, File_Path, id , status FROM team"; 
    ...
    <div class="container-fluid">
        <div class="card">
            <div class="card-body">
                ...
                <div class="row">
                    <a href="create.php" class="btn btn-outline-primary btn-sm ">Create Team Memeber</a>
                </div>
            </div>
        </div>
    </div>
...
<script>
...
}).buttons().container().appendTo('#teamTable_wrapper .col-md-6:eq(0)');

    // Handle delete action
    $('#teamTable tbody').on('click', '.delete-action', function (e) {
    e.preventDefault();
    var id = $(this).data('id');
    console.log(id);
            ...
            success: function (response) {
                try {
                    ...
                    if (result.success) {
                        ...
                        $('#teamTable').DataTable().ajax.reload(); // Refresh the table
    ...
    $('#teamTable tbody').on('click', '.deactivate-action', function (e) {
        e.preventDefault();
        var id = $(this).data('id');
    ...
    $('#teamTable tbody').on('click', '.activate-action', function (e) {
        e.preventDefault();
        var id = $(this).data('id');
        // Confirm before deactivation
